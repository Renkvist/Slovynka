...
try:
	...
	c.execute("UPDATE Users SET current = ? WHERE user_id = ?", (1, 1))
	c.execute("SELECT name FROM Users WHERE current = ?", (1,))
	text = c.fetchone()[0]
...
font_combo = ctk.CTkComboBox(frames["settings"], values=list(font_names.values()), command=change_font)
...
light_butn = ctk.CTkButton(frame_theme, text="", image=sun_icon, width=100, height=60, command=lambda: toggle_theme("light"))
light_butn.pack(side="left", padx=20)
dark_butn = ctk.CTkButton(frame_theme, text="", image=moon_icon, width=100, height=60, command=lambda: toggle_theme("dark"))
dark_butn.pack(side="right", padx=20)

create_labeled_label(frames["settings"], 11, BUTTON_FONT)
language_var = ctk.StringVar()
lang_combo = ctk.CTkComboBox(frames["settings"], values=["ukr", "eng"], command=change_language, variable=language_var)
lang_combo.set(current_language)
lang_combo.pack()
